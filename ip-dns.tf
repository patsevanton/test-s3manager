...
resource "yandex_vpc_address" "addr" {
  name = "s3manager-pip"  # Имя ресурса внешнего IP-адреса

  external_ipv4_address {
    zone_id = yandex_vpc_subnet.s3manager-a.zone  # Зона доступности, где будет выделен IP-адрес
  }
}
...
resource "yandex_dns_zone" "apatsev-org-ru" {
  ...
  private_networks = [yandex_vpc_network.s3manager.id]
}

# Создание DNS-записи типа A, указывающей на внешний IP
resource "yandex_dns_recordset" "s3manager" {
  zone_id = yandex_dns_zone.apatsev-org-ru.id       # ID зоны, к которой принадлежит запись
  name    = "s3manager.apatsev.org.ru."                # Полное имя записи (поддомен)
