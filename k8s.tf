...
resource "yandex_kubernetes_cluster" "s3manager" {
  name       = "s3manager"  # Имя кластера
  network_id = yandex_vpc_network.s3manager.id  # Сеть, к которой подключается кластер

  master {
    version = "1.30"  # Версия Kubernetes мастера
    zonal {
      zone      = yandex_vpc_subnet.s3manager-a.zone  # Зона размещения мастера
      subnet_id = yandex_vpc_subnet.s3manager-a.id     # Подсеть для мастера
    }
...
resource "yandex_kubernetes_node_group" "k8s-node-group" {
  ...
  name        = "k8s-node-group"
  cluster_id  = yandex_kubernetes_cluster.s3manager.id
  ...
  allocation_policy {
    ...
    location { zone = yandex_vpc_subnet.s3manager-a.zone }
    location { zone = yandex_vpc_subnet.s3manager-b.zone }
    location { zone = yandex_vpc_subnet.s3manager-d.zone }
  }
  ...
  instance_template {
    ...
    network_interface {
      ...
      subnet_ids = [
        yandex_vpc_subnet.s3manager-a.id,
        yandex_vpc_subnet.s3manager-b.id,
        yandex_vpc_subnet.s3manager-d.id
      ]
    }
...
provider "helm" {
  kubernetes {
    host                   = yandex_kubernetes_cluster.s3manager.master[0].external_v4_endpoint  # Адрес API Kubernetes
    cluster_ca_certificate = yandex_kubernetes_cluster.s3manager.master[0].cluster_ca_certificate  # CA-сертификат
...
resource "helm_release" "ingress_nginx" {
  ...
  depends_on       = [yandex_kubernetes_cluster.s3manager]
...
output "k8s_cluster_credentials_command" {
  value = "yc managed-kubernetes cluster get-credentials --id ${yandex_kubernetes_cluster.s3manager.id} --external --force"
}
